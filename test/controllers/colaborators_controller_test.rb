...
class ColaboratorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create colaborator' do
    assert_difference('Colaborator.count') do
      post colaborators_url,
           params: { colaborator: { adress: @colaborator.adress, email: @colaborator.email, lastname: @colaborator.lastname,
                                    name: @colaborator.name, number: @colaborator.number, profile: @colaborator.profile, status: @colaborator.status } }
    end
  ...
  test 'should update colaborator' do
    patch colaborator_url(@colaborator),
          params: { colaborator: { adress: @colaborator.adress, email: @colaborator.email, lastname: @colaborator.lastname,
                                   name: @colaborator.name, number: @colaborator.number, profile: @colaborator.profile, status: @colaborator.status } }
